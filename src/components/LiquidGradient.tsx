...
// TouchTexture class to handle mouse/touch trail
class TouchTexture {
  size: number;
  width: number;
  height: number;
...
void main() {
  ...
  float ripple = sin(d * 20.0 - uTime * 3.0) * 0.04 * touchIntensity;
  float wave = sin(d * 15.0 - uTime * 2.0) * 0.03 * touchIntensity;
  uv += vec2(ripple + wave);
...
interface LiquidGradientProps {
  variant?: 'schema1' | 'schema2';
  className?: string;
}

const LiquidGradient: React.FC<LiquidGradientProps> = ({ variant = 'schema1', className }) => {
  ...
  useEffect(() => {
    ...
    // Uniforms setup based on variant
    const isSchema1 = variant === 'schema1';

    const uniforms = {
      uTime: { value: 0 },
      uResolution: { value: new THREE.Vector2(container.clientWidth, container.clientHeight) },
      uColor1: { value: isSchema1 ? new THREE.Vector3(0.945, 0.353, 0.133) : new THREE.Vector3(1.0, 0.424, 0.314) },
      uColor2: { value: isSchema1 ? new THREE.Vector3(0.039, 0.055, 0.153) : new THREE.Vector3(0.251, 0.878, 0.816) },
      uColor3: { value: isSchema1 ? new THREE.Vector3(0.945, 0.353, 0.133) : new THREE.Vector3(1.0, 0.424, 0.314) },
      uColor4: { value: isSchema1 ? new THREE.Vector3(0.039, 0.055, 0.153) : new THREE.Vector3(0.251, 0.878, 0.816) },
      uColor5: { value: isSchema1 ? new THREE.Vector3(0.945, 0.353, 0.133) : new THREE.Vector3(1.0, 0.424, 0.314) },
      uColor6: { value: isSchema1 ? new THREE.Vector3(0.039, 0.055, 0.153) : new THREE.Vector3(0.251, 0.878, 0.816) },
      uSpeed: { value: 0.5 },
      uIntensity: { value: 1.8 },
      uTouchTexture: { value: touchTexture.texture },
      uGrainIntensity: { value: 0.08 },
      uZoom: { value: 1.0 },
      uDarkNavy: { value: isSchema1 ? new THREE.Vector3(0.039, 0.055, 0.153) : new THREE.Vector3(0.02, 0.03, 0.08) },
      uGradientSize: { value: isSchema1 ? 0.45 : 1.0 },
      uGradientCount: { value: isSchema1 ? 12.0 : 6.0 },
      uColor1Weight: { value: isSchema1 ? 0.5 : 1.0 },
      uColor2Weight: { value: isSchema1 ? 1.8 : 1.0 },
    };
  ...
  return (
    <div 
      ref={containerRef} 
      className={`absolute inset-0 z-0 opacity-100 pointer-events-none ${className || ''}`}
    />
  );
};
